...
export default function TrueFalseQuiz({
  ...
  // answers: map từ chỉ số mệnh đề (0-3) sang boolean đã chọn
  const [answers, setAnswers] = useState<Record<number, boolean>>({});
  const [revealed, setRevealed] = useState(false);
  const [scores, setScores] = useState<number[]>([]); // Lưu số ý đúng từng câu
  ...
  const handleCheck = () => {
    ...
    // Tính số ý đúng trong câu hiện tại
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-6 sm:px-6">
      ...
      <div className="flex items-center justify-between gap-3 border-b border-star/10 pb-3">
        <button
          onClick={onBack}
          className="rounded-full border border-star/20 bg-void-card px-3.5 py-1.5 font-mono text-xs text-star hover:border-leaf/40 transition"
        >
          ← Menu bài học
        </button>
        <span className="font-mono text-xs text-leaf-deep">
      ...
      {!isDone ? (
        <div className="mt-6 animate-pop-in">
          <div className="rounded-2xl border border-star/10 bg-void-card p-5 sm:p-6 shadow-card">
            <span className="rounded bg-leaf/15 px-2.5 py-0.5 font-mono text-xs font-semibold text-leaf">
              Tư duy phản biện Đúng / Sai GDPT 2018
            </span>

            {/* Lời dẫn tình huống */}
            <div className="mt-3 rounded-xl border border-star/10 bg-[#07131b] p-4 text-[15px] leading-relaxed text-star">
              <p className="font-semibold text-leaf-deep">Ngữ cảnh thực tiễn trồng trọt:</p>
              <p className="mt-1 text-star/90">{currentQ.context}</p>
            </div>

            {currentQ.code && (
              <pre className="mt-3 overflow-x-auto rounded-xl border border-star/10 bg-[#07131b] p-3 font-mono text-xs text-leaf-deep">
                  ...
                  >
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="flex-1 text-sm sm:text-[15px] leading-relaxed text-star">
                        <strong className="mr-1.5 font-mono text-base font-bold text-leaf">
                          {label})
                        </strong>
                        {st.text}
                      </div>
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        <button
                          ...
                          className={`rounded-xl px-4 py-1.5 font-display text-xs font-bold transition ${
                            pickedVal === true
                              ? "bg-leaf text-void font-extrabold shadow-sm ring-2 ring-leaf-deep"
                              : "border border-star/20 bg-void-card text-star-soft hover:border-leaf/40"
                    ...
                    {revealed && (
                      <div className="mt-3 border-t border-white/5 pt-2 text-xs sm:text-sm">
                        <p className="font-semibold">
                          ...
                          {" — "}
                          {isRight ? (
                            <span className="text-leaf-deep">Bạn đã trả lời đúng!</span>
                          ) : (
            ...
            <div className="mt-6 flex justify-end">
              {!revealed ? (
                <button
                  ...
                  className={`rounded-full px-6 py-2.5 font-display text-sm font-bold text-void shadow-card transition ${
                    allAnswered
                      ? "bg-gradient-to-r from-leaf to-sea-deep hover:shadow-glow cursor-pointer font-extrabold"
                ...
                <button
                  onClick={handleNext}
                  className="rounded-full bg-gradient-to-r from-leaf to-sea-deep px-6 py-2.5 font-display text-sm font-extrabold text-void shadow-card hover:shadow-glow transition"
                >
        ...
        <div className="mt-8 animate-pop-in rounded-2xl border border-star/15 bg-void-card p-6 text-center shadow-card sm:p-8">
          ...
          <div className="flex flex-col items-center justify-center gap-3 sm:flex-row">
            <button
              ...
              className="w-full sm:w-auto rounded-full bg-void-light border border-star/20 px-6 py-2.5 font-display text-sm font-bold text-star hover:bg-void transition"
            >
            ...
            {onGoNext && (
              <button
                onClick={onGoNext}
                className="w-full sm:w-auto rounded-full bg-gradient-to-r from-leaf to-sea px-6 py-2.5 font-display text-sm font-bold text-void shadow-card hover:shadow-glow transition"
              >
                Chuyển sang Tự luận/Vận dụng →
              </button>
            )}
            <button
              onClick={onBack}
