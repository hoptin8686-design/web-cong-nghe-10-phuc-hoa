...
function QuickCheck({
  ...
  return (
    <div className="my-5 rounded-2xl border-2 border-dashed border-leaf/40 bg-leaf/[0.06] p-4 sm:p-5">
              ...
              className={`flex w-full items-start gap-2.5 rounded-xl border-2 px-3.5 py-2.5 text-left text-sm transition ${
                reveal && isRight
                  ...
                  : chosen
                    ...
                    : "border-star/10 bg-void-card text-star hover:border-leaf/50"
      ...
      {picked !== null && (
        <div className="mt-3 animate-pop-in rounded-xl bg-void-card p-3.5 border border-white/5">
          <p className={`font-display text-sm font-semibold ${correct ? "text-leaf-deep" : "text-coral"}`}>
            {correct ? "🎉 Chính xác rồi!" : "💡 Chưa chính xác, cùng xem giải thích nhé:"}
          </p>
...
function RenderBlock({ block }: { block: TheoryBlock }) {
  switch (block.kind) {
    ...
    case "cards": {
      const tone = TONE[block.tone ?? "leaf"];
      return (
      ...
      return (
        <div className="my-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="rounded-xl border border-leaf/30 bg-leaf/[0.06] p-4">
            <div className="flex items-center gap-2 border-b border-leaf/20 pb-2">
              <span className="text-xl">{block.left.emoji}</span>
              <h4 className="font-display font-bold text-sm text-leaf-deep">{block.left.title}</h4>
            </div>
        ...
        <div className="my-4 space-y-3">
          {block.items.map((step, idx) => (
            <div key={idx} className="flex items-start gap-3 rounded-xl border border-star/10 bg-void-card p-3.5">
              <span className="shrink-0 rounded-lg bg-leaf px-2.5 py-1 font-mono text-xs font-bold text-void">
                {step.label}
              </span>
              <div>
    ...
    case "code":
      return (
        <div className="my-4 overflow-hidden rounded-xl border border-star/15 bg-[#07131b]">
          {block.caption && (
            <div className="border-b border-star/10 bg-void-card/60 px-4 py-1.5 font-mono text-xs text-star-soft">
              {block.caption}
            </div>
          )}
          <pre className="overflow-x-auto p-4 font-mono text-xs sm:text-sm text-leaf-deep">
      ...
      return (
        <div className="my-4 rounded-xl border-l-4 border-berry bg-berry/[0.08] p-4 text-star">
          <div className="flex items-center gap-1.5 font-display text-sm font-bold text-berry">
            <span>⚠️</span>
            <span>Lưu ý quan trọng: {block.title}</span>
          </div>
          <p className="mt-1.5 text-xs sm:text-sm leading-relaxed text-star/90">
            <Rich text={block.text} />
          </p>
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      ...
      <div className="flex items-center justify-between gap-2 border-b border-star/10 pb-4">
        <button
          onClick={onBack}
          className="flex items-center gap-1.5 rounded-full border border-star/20 bg-void-card px-3.5 py-1.5 font-mono text-xs font-medium text-star hover:border-leaf/50 hover:bg-void-light transition"
        >
          ← Menu bài học
        </button>
      ...
      {/* Tiêu đề bài */}
      <div className="mt-6">
        <span className="rounded-full bg-leaf/15 px-3 py-1 font-mono text-xs font-semibold text-leaf-deep">
          {topicName}
        </span>
        <h1 className="mt-2.5 font-display text-2xl font-bold text-star sm:text-3xl">
          {lessonTitle}
        </h1>
        <div className="mt-3 rounded-xl border border-leaf/20 bg-leaf/[0.05] p-3.5 text-sm leading-relaxed text-star-soft">
      ...
      {theory.summary && theory.summary.length > 0 && (
        <div className="mt-8 rounded-2xl border border-leaf/30 bg-leaf/[0.07] p-5 sm:p-6 shadow-card">
          <h3 className="flex items-center gap-2 font-display text-base font-bold text-leaf-deep">
            <span>🎯</span>
            <span>Tóm tắt ghi nhớ nhanh:</span>
          </h3>
      ...
      <div className="mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row">
        <button
          ...
          className="w-full sm:w-auto rounded-full bg-gradient-to-r from-leaf to-sea px-6 py-3 font-display text-base font-bold text-void shadow-card hover:shadow-glow transition hover:-translate-y-0.5"
        >
